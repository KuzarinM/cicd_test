<template>
  <div v-if="name" :class="`group-list  ${isCollapsed?'--collapsed':''} ${!isScenarios&&`--child-${childDepth || 0}`} `">
    ...
    <div v-if="isAutomation" class="subgroup-item__service-list">
      <scenario-service
        v-for="device in filteredDevices()"
        :id="device.id"
        :key="device.id"
        ...
        @update-capability="e=>{emit('update',e)}"
        @on-off="e => {emit('onOffDevice',e)}"
      />
    </div>
...
const filteredDevices = () => {
  ...
  if (props.hideSensors && typeof props.hideSensors !== 'undefined') {
    if (temp?.length) {
      temp = temp?.filter(el => !(el.type?.includes('sensor') || el.type?.includes('smart_meter')))
      return temp
    }
    temp = props.devices?.filter(el => !(el.type.includes('sensor') || el.type?.includes('smart_meter'))) || []
  }
  return temp
}
